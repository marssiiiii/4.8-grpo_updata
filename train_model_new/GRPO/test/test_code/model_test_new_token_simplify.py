...
# ------------------------
TRAIN_STEP=9000
MODEL_PATH=f"five_plus_two_optimization/train_model_new/GRPO/save_model/grpo_fpt_api/grpo_fpt_api_2/step_{TRAIN_STEP}" #var
#MODEL_PATH="/home/jiuxing_li/five_plus_two_optimization/train_model_new/GRPO/base_model/ernie_base_model_9"
...
OUTPUT_PATH_TRAIN=f"five_plus_two_optimization/train_model_new/GRPO/test/test_jsonl/grpo_fpt_api_2/simplify/grpo_fpt_api_2_{TRAIN_STEP}step_train.jsonl" #var
OUTPUT_PATH_TEST=f"five_plus_two_optimization/train_model_new/GRPO/test/test_jsonl/grpo_fpt_api_2/simplify/grpo_fpt_api_2_{TRAIN_STEP}step_test.jsonl"
#OUTPUT_PATH_TRAIN="five_plus_two_optimization/train_model_new/GRPO/test/test_jsonl/ernie_base_model_9_simplify_train.jsonl" #var
#OUTPUT_PATH_TEST="five_plus_two_optimization/train_model_new/GRPO/test/test_jsonl/ernie_base_model_9_simplify_test.jsonl"
MAX_LENGTH=10000
...
if __name__ == "__main__":
    ...
    NEED_ASK_TRAIN,NEED_ASK_TEST=True,True

    if NEED_ASK_TRAIN:
        if NEED_OUTPUT:
            gen_sample(total_num=TRAIN_NUM,sample_points=train_sample_points,data_json=TRAIN_JSON,num_tokens=NUM_TOKENS,output_path=OUTPUT_PATH_TRAIN)
        else:
            gen_sample(total_num=TRAIN_NUM,sample_points=train_sample_points,data_json=TRAIN_JSON,num_tokens=NUM_TOKENS)
    if NEED_ASK_TEST:
        if NEED_OUTPUT:
            gen_sample(total_num=TEST_NUM,sample_points=test_sample_points,data_json=TEST_JSON,num_tokens=NUM_TOKENS,output_path=OUTPUT_PATH_TEST) 
        else:
