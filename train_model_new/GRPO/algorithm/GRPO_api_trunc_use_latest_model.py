...
class Algorithm(AlgorithmBase):
    ...
    def gen_worker(Q, model_path: str, gen_device: int, ref_server_url: str,
        ...
        def reward_correct(context_list,completion_predict_list,response_list,house,floor,bound,pre_post_text,pre_lineload_text,design,gen_num):
            ...
            "floor_price_list":[],"diaphragm_area_ratio_list":[],"is_improved":[],"design_error_score_list":[],"design_invalid_process_score_list":[],
            "design_diaphragm_score_list":[],"design_dcr_score_list":[],"design_price_score_list":[]}
        ...
        for it in range(999999999):
            ...
            while NUM_GENERATE<Q_batch_size:
                i=NUM_GENERATE
                if len(gen_sample_return_result["score_list"])<(i+1)*num_pre_Q:
                    print(f"gen_sample生成的样本数不足{(i+1)*num_pre_Q}")
                    NUM_GENERATE+=1
                    continue
    ...
    def step(self, batch: Dict,curr_step:int,log_file:str,rank:int) -> torch.Tensor:
        ...
        latest_per_token_logps = batch['latest_refs'].to(per_token_logps.device)

        #d = ref_per_token_logps - per_token_logps #d为ref模型的log π(a|s)-现有模型的log π(a|s)
        d = latest_per_token_logps - per_token_logps #d为ref模型的log π(a|s)-现有模型的log π(a|s)
