...
from reward_design import process_completion_predict_based_on_response,get_design_score,get_per_token_score
...
class Algorithm(AlgorithmBase):
    ...
    def gen_worker(Q, model_path: str, gen_device: int, ref_server_url: str,
        ...
        vllm_gen = LLM(model=model_path, gpu_memory_utilization=0.5) #gpu_memory_utilization指的是vLLM 在「每一张 GPU 上」允许占用的显存比例上限
        print("gen_worker初始化vllm_gen成功")
        ref_server_ver = 'tensor'

        sampling_params = SamplingParams(n=num_pre_Q, temperature=0.8, max_tokens=200) #生成问题答案的参数(生成答案数量,多样性,token数限制) #var
        ...
        def gen_answers(prompt_list):
            ...
            response_list, response_encoded_list = [], []
            for v in voutputs: #将所有生成的answer都打平,返回answer+answer_token_id
                for z in v.outputs:
                    response=tokenizer.decode(z.token_ids)
                    response_encoded=tokenizer(response,return_offsets_mapping=True)
                    response_list.append(response)
                    response_encoded_list.append(response_encoded)
            return response_list,response_encoded_list

        # 奖励函数分两部分：正确性奖励+格式奖励
        # 正确性奖励:当前的逻辑是依据无支撑次数和房间数误差进行打分
        def reward_correct(context_list,completion_predict_list,response_list,response_encoded_list,house,floor):
            valid_area_ratio_list,reward_score_list=[],[]
            is_improved=[]
            for i in range(len(context_list)):
                context,completion_predict,response,response_encoded=\
                    context_list[i],completion_predict_list[i],response_list[i],response_encoded_list[i]
                ...
                response_length,response_offset_mapping = len(response_encoded["input_ids"]),response_encoded['offset_mapping']
                per_token_score=get_per_token_score(context=context,completion_predict=completion_predict,response=response,
                            response_offset_mapping=response_offset_mapping,response_length=response_length)
                reward_score_list.append(per_token_score)
                valid_area_ratio_list.append(result['area_ratio'])
                if result['area_ratio']>result_before['area_ratio']:
                    is_improved.append(1)
                else:
                    is_improved.append(0)
                if i==0:
                    print(f"house:{house},floor:{floor},response:{response},per_token_score:{per_token_score}")
            print(f"house:{house},floor:{floor},valid_area_ratio_list:{valid_area_ratio_list},is_improved:{is_improved}")
            return reward_score_list,is_improved
        ...
        def gen_samples(inputs):
            ...
            response_list, response_encoded_list = gen_answers(prompt_list) #依据inputs提问生成答案
            rewards,is_improved = [],[] #依据答案生成reward,标准的prompt_ids
            for i, inp in enumerate(inputs):
                sample_context_list,sample_completion_predict_list,sample_response_list,sample_response_encoded_list=[],[],[],[]
                for response,response_encoded in zip(
                    response_list[i*num_pre_Q:(i+1)*num_pre_Q],response_encoded_list[i*num_pre_Q:(i+1)*num_pre_Q]):
                    ...
                    sample_response_encoded_list.append(response_encoded)
                batch_rewards,batch_is_improved=reward_correct(sample_context_list,sample_completion_predict_list
                    ,response_list=sample_response_list,response_encoded_list=sample_response_encoded_list,
                    house=inp["house"],floor=inp["floor"])
            ...
            return prompt_list,house_list,floor_list,rewards_tensor,response_list,response_encoded_list,is_improved
        ...
        def check_and_update_model(it):
            ...
            try:
                ...
                if new_version > current_model_version and model_path_new:
                    ...
                    vllm_gen = LLM(model=model_path_new, gpu_memory_utilization=0.5)
        ...
        for it in range(999999999):
            ...
            tic = time.time()
            prompt_list,house_list,floor_list, rewards, response_list, response_encoded_list,is_improved = gen_samples(inputs) #依据采样问题得到采样答案
            ...
            while NUM_GENERATE<Q_batch_size:
                ...
                #求得curr_rewards进行标准化
                flat_rewards = torch.cat([
                    torch.as_tensor(r, dtype=torch.float32)
                    for r in curr_rewards
                ])
                mean_r,std_r = flat_rewards.mean(),flat_rewards.std()
                if np.sum(curr_is_improved)<5:
                    print("显著提升原设计样本不足！")
                    inputs = random.sample(QAs,Q_batch_size) #采样问题
                    #prompt_list,house_list,floor_list, rewards, response_list, response_encoded_list = gen_samples(inputs) #重新采样
                    NUM_GENERATE=NUM_GENERATE+1
                    continue
                '''
                ANSWER_IS_INVALID=0
                for item_index,reward_item in enumerate(curr_rewards): #检测一批reward内有无分数全部一致的答案，有则该批次答案无效
                    max_r,min_r=reward_item.max(),reward_item.min()
                    #if max_r-min_r<1e-4:
                    if max_r<=-1:
                        print(f"不符合规定的reward_{item_index}")
                        ANSWER_IS_INVALID=ANSWER_IS_INVALID+1
                if ANSWER_IS_INVALID>10: #var
                    print("奖励无差异！")
                    inputs = random.sample(QAs,Q_batch_size) #采样问题
                    #prompt_list,house_list,floor_list, rewards, response_list, response_encoded_list = gen_samples(inputs) #重新采样
                    NUM_GENERATE=NUM_GENERATE+1
                ...
                flat_rewards_std = (flat_rewards - mean_r) / (std_r + 1e-4)
                curr_rewards_std = []
                idx = 0
                for r in curr_rewards:
                    n = len(r)
                    curr_rewards_std.append(flat_rewards_std[idx:idx+n])
                    idx += n

                if ref_server_ver == 'tensor':
                    for ii in range(0, num_pre_Q, train_batch_size):
                        sub_rewards=curr_rewards[ii:ii+train_batch_size]
                        sub_rewards_std = curr_rewards_std[ii:ii+train_batch_size]
                        sub_rewards,sub_rewards_std=pad_sequence(sub_rewards, batch_first=True, padding_value=tokenizer.pad_token_id),\
                        pad_sequence(sub_rewards_std, batch_first=True, padding_value=tokenizer.pad_token_id)
                        ...
                        data = [json.dumps({"house":house,"floor":floor,"plen": plen}).encode(), tensor_to_bytes(merged_ids), tensor_to_bytes(sub_rewards_std),tensor_to_bytes(sub_rewards)]
                    ...
                    xdata = make_bytes_list([json.dumps({"Q": pp[0], "As": curr_response_list}).encode(),
