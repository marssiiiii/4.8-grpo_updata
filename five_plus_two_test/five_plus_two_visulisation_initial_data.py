...
def plot_diaphragms(ax,poly_list,POLYGON_TYPES,LINE_TYPES,POINT_TYPES,bound,title="",code=0):
    ...
    # 绘制网格
    ax.set_xticks(range(int(bound[0]), int(bound[1]), 1024))
    ax.set_yticks(range(int(bound[2]), int(bound[3]), 256))
...
def plot_segments(ax, segment_list,POLYGON_TYPES,LINE_TYPES,POINT_TYPES,bound,cut_line_list=None,title="",code=0):
    bound=[bound[0],bound[1],bound[2],bound[3]]
    ax.set_xlim(int(bound[0]), int(bound[1]))
    ax.set_ylim(int(bound[2]), int(bound[3]))
    ax.set_aspect("equal", adjustable="box")
    ...
    # 绘制网格
    ax.set_xticks(range(int(bound[0]), int(bound[1]), 1024))
    ax.set_yticks(range(int(bound[2]), int(bound[3]), 1024))
    ax.grid(True, linestyle='--', alpha=0.4)
...
def visualize_segments_five_ax(segment_list,beams,shear_walls,valid_beams,valid_shear_walls,output_dir,poly_list,valid_poly_list,bound):
...
if __name__=="__main__":
    load_path="train_json_data/five_plus_two_train_jsonl_data/design_3.27/initial_data_100_train_set_inbox_shf.jsonl"
    with open(load_path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            shear_walls=result["shear_walls"]

            #print(segment_list)
            
            print(f"Visualizing record {i+1}")
            output_dir=f"five_plus_two_optimization/five_plus_two_test/test_pic/data_3.27/initial_inbox_data/sample_{i+1}_{house}_{floor}.png"
            visualize_segments_five_ax(segment_list,beams=result["beams"],shear_walls=result["shear_walls"],
